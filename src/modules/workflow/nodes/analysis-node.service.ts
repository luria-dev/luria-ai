  ...
  async analyze(input: AnalyzeInput): Promise<AnalysisOutput> {
    ...
    const context: AnalysisPromptContext = {
      ...
      outputPolicy: {
        ...
        doNotOverrideStrategyVerdict: true,
        comparisonMode:
    ...
    const prompts = buildAnalysisPrompts(context);
  ...
  private buildDeterministicAnalysis(input: AnalyzeInput): AnalysisOutput {
    ...
    if (typeof price.change1hPct === 'number') {
      keyObservations.push(
        `1h price change is ${price.change1hPct.toFixed(2)}%.`,
      );
    }
    if (typeof price.change24hPct === 'number') {
      keyObservations.push(
        `24h price change is ${price.change24hPct.toFixed(2)}%.`,
      );
    } else {
    ...
    if (typeof price.change7dPct === 'number') {
      keyObservations.push(
        `7d price change is ${price.change7dPct.toFixed(2)}%.`,
      );
    }
    if (typeof price.change30dPct === 'number') {
      keyObservations.push(
        `30d price change is ${price.change30dPct.toFixed(2)}%.`,
      );
    }
    keyObservations.push(
    ...
    if (input.strategy.verdict === 'BUY') {
      opportunityHighlights.push(
        'No critical risk block and signals align to upside.',
      );
    } else if (input.strategy.verdict === 'HOLD') {
      opportunityHighlights.push(
        'Mixed market signals suggest waiting for confirmation.',
      );
    } else if (input.strategy.verdict === 'CAUTION') {
      opportunityHighlights.push(
        'Potential setup exists but warning-level risk is active.',
      );
    }
