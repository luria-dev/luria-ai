...
export class LlmRuntimeService {
  ...
  async generateStructured<T>(input: GenerateStructuredInput<T>): Promise<T> {
    ...
    if (!apiKey) {
      this.logger.warn(`[${input.nodeName}] OPENAI_API_KEY is missing, fallback to mock.`);
      return input.fallback();
    }

    try {
      const first = await this.callOpenAi(apiKey, input.systemPrompt, input.userPrompt);
      const firstParsed = this.parseStructured(first, input.schema);
      if (firstParsed.success) {
        return firstParsed.data;
      }

      this.logger.warn(`[${input.nodeName}] First pass schema invalid, retrying with correction prompt.`);
      ...
      this.logger.warn(`[${input.nodeName}] Retry schema still invalid, fallback to mock.`);
      return input.fallback();
    } catch (error) {
  ...
  private async callOpenAi(apiKey: string, systemPrompt: string, userPrompt: string): Promise<string> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`OpenAI request failed: ${response.status} ${response.statusText} ${errText}`);
    }
