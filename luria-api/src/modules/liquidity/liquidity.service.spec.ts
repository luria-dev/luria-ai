...
describe('LiquidityService.fetchSnapshot', () => {
  ...
  it('should compute 1h drop from in-memory baseline and trigger withdrawal risk', async () => {
    ...
    const service = new LiquidityService();
    (service as unknown as { liquidityHistory: Map<string, Array<{ atMs: number; liquidityUsd: number }>> })
      .liquidityHistory.set('ethereum:0xpair', [
      {
    ...
    expect(snapshot.sourceUsed).toBe('dexscreener');
  });
});
