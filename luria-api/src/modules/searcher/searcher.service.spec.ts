...
import type { AnalyzeCandidate } from '../../core/contracts/analyze-contracts';

const makeCandidate = (symbol: string, chain: string, pair: string): AnalyzeCandidate => ({
  candidateId: `cand-${chain}-${pair}`,
  tokenName: symbol,
  symbol,
  chain,
  ...
  it('should split targets from intent entities when taskType is comparison', async () => {
    const marketStub: Pick<MarketService, 'searchCandidates'> = {
      async searchCandidates(query: string): Promise<AnalyzeCandidate[]> {
        if (query === 'ASTER') {
          return [makeCandidate('ASTER', 'base', 'pair-aster')];
        }
        if (query === 'HYPER') {
          return [makeCandidate('HYPER', 'arbitrum', 'pair-hyper')];
        }
        return [];
      },
    };
    ...
    const result = await service.resolveMany('Aster vs Hyper 谁更值得投资', null, {
      taskType: 'comparison',
